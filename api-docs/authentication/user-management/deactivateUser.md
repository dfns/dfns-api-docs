...
{% code title="deactivateUser.ts" overflow="wrap" lineNumbers="true" %}
```typescript
...
const resourcePath = '/auth/users/us-2mhcm-9r90a-92ran47bjpl60hmv/deactivate'
...
{% code title="deactivateUser.js" overflow="wrap" lineNumbers="true" %}
```javascript
import { httpClient } from './dfnsHttpClient'
import { generateUserActionSignature } from './generateUserActionSignature'
...
{% tab title="Curl" %}
{% code title="deactivateUser.sh" overflow="wrap" lineNumbers="true" %}
```shell
currentTime=$( date -u +"%Y-%m-%dT%H:%M:%SZ" )
nonce=$( echo "{\"date\":\"$currentTime\",\"uuid\":\"$(uuidgen)\"}" | base64 | tr '/+' '_-' | tr -d '=' )

curl -X PUT "/auth/users/us-2mhcm-9r90a-92ran47bjpl60hmv/deactivate" \
