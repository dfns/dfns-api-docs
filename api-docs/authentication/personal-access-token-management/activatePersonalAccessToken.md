...
{% code title="activatePersonalAccessToken.ts" overflow="wrap" lineNumbers="true" %}
```typescript
...
const resourcePath = '/auth/pats/to-7vwr2-h2f29-9m6a0fmkk7rr2hj7/activate'
generateUserActionSignature(
  ...
  httpClient.put<AccessTokenInfoWithPublicKey>(
    ...
    console.error(`Failed to activate personal access token: ${error.message}`)
  })
}).catch((error) => {
...
{% code title="activatePersonalAccessToken.js" overflow="wrap" lineNumbers="true" %}
```javascript
import { httpClient } from './dfnsHttpClient'
import { generateUserActionSignature } from './generateUserActionSignature'
...
{% tab title="Curl" %}
{% code title="activatePersonalAccessToken.sh" overflow="wrap" lineNumbers="true" %}
```shell
currentTime=$( date -u +"%Y-%m-%dT%H:%M:%SZ" )
nonce=$( echo "{\"date\":\"$currentTime\",\"uuid\":\"$(uuidgen)\"}" | base64 | tr '/+' '_-' | tr -d '=' )

curl -X PUT "/auth/pats/to-7vwr2-h2f29-9m6a0fmkk7rr2hj7/activate" \
