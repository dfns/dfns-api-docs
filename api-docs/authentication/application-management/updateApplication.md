...
**Success** - The application that was updated
...
{% code title="updateApplication.ts" overflow="wrap" lineNumbers="true" %}
```typescript
...
{% code title="updateApplication.js" overflow="wrap" lineNumbers="true" %}
```javascript
import { httpClient } from './dfnsHttpClient'
import { generateUserActionSignature } from './generateUserActionSignature'
...
{% tab title="Curl" %}
{% code title="listApplications.sh" overflow="wrap" lineNumbers="true" %}
```shell
currentTime=$( date -u +"%Y-%m-%dT%H:%M:%SZ" )
nonce=$( echo "{\"date\":\"$currentTime\",\"uuid\":\"$(uuidgen)\"}" | base64 | tr '/+' '_-' | tr -d '=' )

appName='My New App Name'
curl -X PUT "/auth/apps/ap-4s6se-e2t7n-89gfg50iaos73pm6" \
