...
{% code title="deactivateApplication.ts" overflow="wrap" lineNumbers="true" %}
```typescript
...
const resourcePath = '/auth/apps/ap-4s6se-e2t7n-89gfg50iaos73pm6/deactivate'
generateUserActionSignature(
  ...
  httpClient.put<AppInfoWithPublicKey>(
    ...
    console.error(`Failed to deactivate application: ${error.message}`)
  })
}).catch((error) => {
...
{% code title="deactivateApplication.js" overflow="wrap" lineNumbers="true" %}
```javascript
import { httpClient } from './dfnsHttpClient'
import { generateUserActionSignature } from './generateUserActionSignature'
...
{% tab title="Curl" %}
{% code title="activateApplication.sh" overflow="wrap" lineNumbers="true" %}
```shell
currentTime=$( date -u +"%Y-%m-%dT%H:%M:%SZ" )
nonce=$( echo "{\"date\":\"$currentTime\",\"uuid\":\"$(uuidgen)\"}" | base64 | tr '/+' '_-' | tr -d '=' )

curl -X PUT "/auth/apps/ap-4s6se-e2t7n-89gfg50iaos73pm6/deactivate" \
